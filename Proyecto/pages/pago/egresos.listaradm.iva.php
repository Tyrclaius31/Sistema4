...
<div id="contenedorgeneral">
 ...
 <form id="form1" name="form1" method="post" action="">
  <div align="center">
    <h3><strong>Informe de Egresos por Mes</strong> Con Iva</h3>
    <div id="tira">
...
<?php
	...
	echo "<b><font color='#000'><h4><div align='left'>Egresos Registrados en $mes del Año $ano</div></h4></font></b> ";
	echo "<hr>";
	echo "<b><font face='Times New Roman' size='1'><table width='670' border='0' align='center'>
  <tr><td width='60'>FECHA</td><td width='85'>N° BOL./FACT.</td><td width='70'>RAZÓN</td><td width='200'>DESCRIPCIÓN</td><td width='45'>NETO</td><td width='45'>IVA</td><td width='55'>TOTAL</td><td width='60'>F. MOD.</td><td width='50'>USUARIO</td></tr></table></font></b>";
	echo "<hr>";
		$sql="select * from egresos_iva where Mes=".$_POST[mes]." and Ano=".$_POST[ano]." order by Dia asc";
		$respuesta=mysql_query($sql,$conexion);
		
		if (mysql_num_rows($respuesta) == 0) {
   			echo '<script>alert("No existen Egresos en este Mes.");</script>';
    	exit;
		}else{
		...
		{
			echo "<tr><td width='60'>".$row["Ano"]."-".$row["Mes"]."-".$row["Dia"]."</td><td width='85'>".$row["N_Boleta"]."</td><td width='70'>".$row["Razon"]."</td><td width='200'>".$row["Descripcion"]."</td><td width='45'>".$row["Valor_Neto"]."</td><td width='45'>".$row["Valor_Iva"]."</td><td width='55'>".$row["Costo"]."</td><td width='60'>".$row["Fecha_Modificacion"]."</td><td width='50'>".$row["Modificacion_Usuario"]."</td></tr>";
		} 
		echo "<h6></table>";
...
?>
    ...
    <hr>
<?
	$sql="select sum(Costo) from egresos_iva where Mes=".$_POST[mes]." and Ano=".$_POST[ano];
		$respuesta=mysql_query($sql,$conexion);
