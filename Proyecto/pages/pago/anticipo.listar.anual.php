...
<div id="contenedorgeneral">
 ...
 <form id="form1" name="form1" method="post" action="">
  <div align="center">
    <h3><strong>Lista de Anticipos Anuales</strong></h3>
    <div id="tira">
  ...
  <option selected="selected">2010</option>
	...
	echo "<b><font color='#000'><h4><div align='left'>Anticipos Entregados en el Año $ano</div></h4></font></b> ";
	echo "<hr>";
  ...
  <tr><td width='100'>FECHA</td><td width='200'>TRABAJADOR</td><td width='100'>RUT</td><td width='100'>CAMIÓN</td><td width='150'>MONTO ANTICIPO</td></tr></table></font></b>";
	echo "<hr>";
		$sql="select * from anticipo where Ano=".$_POST[ano];
		$respuesta=mysql_query($sql,$conexion);
		
		if (mysql_num_rows($respuesta) == 0) {
   			echo '<script>alert("No existen Anticipos en este Año.");</script>';
    	exit;
		}else{
...
?>
    ...
    <hr />
<?
	$sql="select sum(Monto) from anticipo where Ano=".$_POST[ano];
		$respuesta=mysql_query($sql,$conexion);
		
		while($row=mysql_fetch_array($respuesta)){
			
		echo "<div align='right'><h4>Total Egresos por Anticipo en el Año $ano  =  $  ".$row['sum(Monto)']."</h4></div>";
