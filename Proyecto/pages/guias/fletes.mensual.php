...
<div id="osx-modal-content">
			...
			<div id="osx-modal-data">
				<h3>Listar Registros de Fletes de Ocación por Mes</h3>
                 <p align="justify">Esta opción nos permite mostrar la lista de fletes realizadas en un mes en espacial. Sólo hay que seleccionar el Mes y el Año, y se desplegará el informe de los Fletes ingresados en la selección, también mostrará la suma total de ingreso ($$) del Mes seleccionado.<br></p>      
...
<div id="contenedorgeneral">
 ...
 <form id="form1" name="form1" method="post" action="">
  <div align="center">
    <h3><strong>Lista de Fletes de Ocación por Mes</strong></h3>
    <div id="tira">
    <table width="376" border="0">
      <tr>
        <td width="95"><h4>Seleccione Mes </h4></td>
...
<option selected="selected">2010</option>
	...
	echo "<b><font color='#000'><h4><div align='left'>Fletes de Ocación de $mes del Año $ano</div></h4></font></b> ";
	echo "<hr>";
  ...
  <tr><td width='60'>FECHA</td><td width='100'>TRABAJADOR</td><td width='70'>CAMIÓN</td><td width='80'>ORIGEN</td><td width='90'>DESTINO</td><td width='70'>V. FLETE</td><td width='65'>% A PAGAR</td><td width='65'>V. A PAGAR</td><td width='70'>TOTAL</td></tr></table></font></b>";
	echo "<hr>";
		$sql="select * from fletes_ocacion where Mes=".$_POST[mes]." and Ano=".$_POST[ano];
		$respuesta=mysql_query($sql,$conexion);
		
		if (mysql_num_rows($respuesta) == 0) {
   			echo '<script>alert("No existen Fletes en este Mes.");</script>';
    	exit;
		}else{
		echo "<h6><table width='670' border='0' align='center'>";
		while($row=mysql_fetch_array($respuesta))
		{
			echo "<tr><td width='60'>".$row["Fecha"]."</td><td width='100'>".$row["Nombre_Tra"]."</td><td width='70'>".$row["Patente_Ca"]."</td><td width='80'>".$row["Origen"]."</td><td width='90'>".$row["Destino"]."</td><td width='70'>".$row["Monto"]."</td><td width='65'>".$row["Porcentaje"]."%</td><td width='65'>".$row["Valor_Por"]."</td><td width='70'>".$row["Total"]."</td></tr>";
		} 
		echo "<h6></table>";
...
?>
    ...
    <hr>
<?
	$sql="select sum(Total) from fletes_ocacion where Mes=".$_POST[mes]." and Ano=".$_POST[ano];
		$respuesta=mysql_query($sql,$conexion);
		
		while($row=mysql_fetch_array($respuesta)){
			
		echo "<div align='right'><h4>Total Ingreso en $mes del $ano  =  $  ".$row['sum(Total)']."</h4></div>";
