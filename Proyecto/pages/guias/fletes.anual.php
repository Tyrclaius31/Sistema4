...
<div id="osx-modal-title"><h2 align="center"><b>Ayuda Auxiliar para Sistema de Gestión</b></h2></div>
			...
			<div id="osx-modal-data">
				<h3>Listar Registros de Fletes de Ocación Anual</h3>
                 <p align="justify">Esta opción nos permite mostrar la lista de fletes realizadas en un año en espacial. Sólo hay que seleccionar el Año, y se desplegará el informe de los Fletes ingresados durante ese periodo, también mostrará la suma total de ingreso ($$) del Año seleccionado.<br></p>      
...
<div id="contenedorgeneral">
 ...
 <form id="form1" name="form1" method="post" action="">
  <div align="center">
    <h3><strong>Lista de Fletes de Ocación por Año</strong></h3>
    <div id="tira">
    <table width="270" border="0">
      <tr>
        <td width="114"><h4>Seleccione Año</h4></td>
        <td width="69"><select name="ano" id="ano">
  <option selected="selected">2010</option>
	...
	echo "<b><font color='#000'><h4><div align='left'>Fletes de Ocación del Año $ano</div></h4></font></b> ";
	echo "<hr>";
		echo "<b><font face='Times New Roman' size='1'><table width='670' border='0' align='center'>
  <tr><td width='60'>FECHA</td><td width='100'>TRABAJADOR</td><td width='70'>CAMIÓN</td><td width='80'>ORIGEN</td><td width='90'>DESTINO</td><td width='70'>V. FLETE</td><td width='65'>% A PAGAR</td><td width='65'>V. A PAGAR</td><td width='70'>TOTAL</td></tr></table></font></b>";
	echo "<hr>";
		$sql="select * from fletes_ocacion where Ano=".$_POST[ano]." order by Mes asc";
		$respuesta=mysql_query($sql,$conexion);
		
		if (mysql_num_rows($respuesta) == 0) {
   			echo '<script>alert("No existen Fletes en este Año.");</script>';
    	exit;
		}else{
		echo "<h6><table width='670' border='0' align='center'>";
		while($row=mysql_fetch_array($respuesta))
		{
			echo "<tr><td width='60'>".$row["Ano"]."-".$row["Mes"]."-".$row["Dia"]."</td><td width='100'>".$row["Nombre_Tra"]."</td><td width='70'>".$row["Patente_Ca"]."</td><td width='80'>".$row["Origen"]."</td><td width='90'>".$row["Destino"]."</td><td width='70'>".$row["Monto"]."</td><td width='65'>".$row["Porcentaje"]."%</td><td width='65'>".$row["Valor_Por"]."</td><td width='70'>".$row["Total"]."</td></tr>";
		} 
		echo "<h6></table>";
...
?>
    ...
    <hr>
<?
	$sql="select sum(Total) from fletes_ocacion where Ano=".$_POST[ano];
		$respuesta=mysql_query($sql,$conexion);
		
		while($row=mysql_fetch_array($respuesta)){
			
		echo "<div align='right'><h4>Total Ingreso en el Año $ano  =  $  ".$row['sum(Total)']."</h4></div>";
