...
<div id="osx-modal-content">
			...
			<div id="osx-modal-data">
				<h3>Vitácora de Mantenciones a Camiones</h3>
                 <p align="justify">Esta opción permite mostrar un listado de todas las Mantenciones realizadas en una determinada fecha.<br><br>Para esto, debe seleccionar el Mes y el Año, y seleccionar el botón <b>"Buscar"</b> y aparecerá una lista con todas las Mantenciones hechas en la fecha selecionada. También en la selección del MES, al final está la opción "TODOS", la cual al seleccionarla le mostrará todo lo realizado en el Año que indique.</p>       
...
<div id="contenedorgeneral">
 ...
 <form id="form1" name="form1" method="post" action="">
  <div align="center">
    <h3><strong>Bitácora de Mantención</strong></h3>
    <div id="tira">
    <table width="410" border="0">
      <tr>
        <td width="141"><strong>Seleccione Mes </strong></td>
        <td width="115"><select name="mes" id="mes">
...
<option selected="selected">2010</option>
	...
	if($_POST[mes]==01){ $mess='Enero';}
	if($_POST[mes]==02){ $mess='Febrero';}
	if($_POST[mes]==03){ $mess='Marzo';}
	if($_POST[mes]==04){ $mess='Abril';}
	if($_POST[mes]==05){ $mess='Mayo';}
	if($_POST[mes]==06){ $mess='Junio';}
	if($_POST[mes]==07){ $mess='Julio';}
	if($_POST[mes]==08){ $mess='Agosto';}
	if($_POST[mes]==09){ $mess='Septiembre';}
	if($_POST[mes]==10){ $mess='Octubre';}
	if($_POST[mes]==11){ $mess='Noviembre';}
	if($_POST[mes]==12){ $mess='Diciembre';}	
	...
	echo "<b><font color='#000'><h4><div align='left'>Vitácora de Mantención de todos los Camiones durante el Año $ano</div></h4></font></b> ";
	echo "<hr>";
	echo "<b><font face='Times New Roman' size='1'><table width='650' border='0' align='center'>
    <tr><td width='70'>FECHA</td><td width='70'>PATENTE</td><td width='140'>CHOFER A CARGO</td><td width='370'>DESCRIPCION</td></tr>    </table></font></b>";
	echo "<hr>";
		
		$sql="select a.Patente_Ca, a.Nombre_Tra, b.Patente_Ca, b.Descripcion, b.Dia, b.Mes, b.Ano, b.Fecha_Ingreso from        asignacion_camiones a, mantencion b where b.Ano=".$_POST[ano]." and a.Patente_Ca=b.Patente_Ca order by b.Fecha_Ingreso and a.Patente_Ca asc";
		$respuesta=mysql_query($sql,$conexion);
		
		if (mysql_num_rows($respuesta) == 0) {
   			echo '<script>alert("No existen Mantenciones para ningún Camión en su Selección.");</script>';
    	exit;
		}else{
		...
		{
			echo "<tr><td width='70'>".$row["Ano"]."-".$row["Mes"]."-".$row["Dia"]."</td><td width='70'>".$row["Patente_Ca"]."</td>            <td width='140'>".$row["Nombre_Tra"]."</td><td width='370'>".$row["Descripcion"]."</td></tr>";
		} 
		echo "</table></h6>";
		...
		echo "<b><font color='#000'><h4><div align='left'>Vitácora de Mantención de Todos los Camiones realizadas en $mess del $ano        </div></h4></font></b> ";
		echo "<hr>";
		echo "<b><font face='Times New Roman' size='1'><table width='650' border='0' align='center'>
    	<tr><td width='70'>FECHA</td><td width='70'>PATENTE</td><td width='140'>CHOFER A CARGO</td><td width='370'>DESCRIPCION</td>	   		</tr></table></font></b>";
		echo "<hr>";	
		
		$sql="select a.Patente_Ca, a.Nombre_Tra, b.Patente_Ca, b.Descripcion, b.Dia, b.Mes, b.Ano, b.Fecha_Ingreso from                asignacion_camiones a, mantencion b where b.Ano=".$_POST[ano]." and b.Mes=".$_POST[mes]." and a.Patente_Ca=b.Patente_Ca order by b.Fecha_Ingreso and b.Patente_Ca asc";
		$respuesta=mysql_query($sql,$conexion);
