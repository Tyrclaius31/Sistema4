  ...
  <div id="cuerpoleft">
   ...
   <form id="form1" name="form1" method="post" action="">
	...
	?>
      <script>document.form1.nombre.focus()</script>
        ...
        <td width="159"><?php
		  ...
		  $result=mysql_query("select * from n_operador where Estado='CONTRATADO'");
		if ($row = mysql_fetch_array($result)){ 
	...
	if($_POST[button2]=="Aceptar")
	{
		...
		from camiones a, n_operador b where a.Id =".$_POST['nombre']." and b.Rut_Operador=".$_POST['nombre1'];
    ...
    <table width="560" border="0" align="center">
      ...
      <tr>
        ...
        <td width="165"><label>
          <input name="marca" type="text" id="marca" value="<? echo $registro[1];?>" size="25" readonly="readonly"/>
        </label></td>
        <td width="87">Modelo</td>
        <td width="156"><label>
          <input name="modelo" type="text" id="modelo" value="<? echo $registro[2];?>" size="25" readonly="readonly"/>
        </label></td>
      </tr>
      <tr>
        <td>Patente</td>
        <td><label>
          <input name="patente" type="text" id="patente" value="<? echo $registro[5];?>" size="25" maxlength="100" readonly="readonly"/>
        </label></td>
        <td>Nombre</td>
        <td><input name="nombre" type="text" id="nombre" value="<? echo $registro[4];?>" size="25" maxlength="25" readonly="readonly"/></td>
      </tr>
      <tr>
        <td colspan="3">&nbsp;</td>
        <td>&nbsp;</td>
      ...
      <tr>
        ...
        <td><input name="id2" type="hidden" id="id2" value="<? echo $registro[6];?>" /></td>
      </tr>
      <tr>
        <td>Nombre</td>
        <td><label>
          <input name="nom" type="text" id="nom" value="<? echo $registro[8];?>" size="25" readonly="readonly" />
        </label></td>
        <td>Rut</td>
        <td><label>
          <input name="rut" type="text" id="rut" value="<? echo $registro[7];?>" size="25" readonly="readonly" />
        </label></td>
      </tr>
      <tr>
        <td>Telefono</td>
        <td><label>
          <input name="telefono" type="text" id="telefono" value="<? echo $registro[9];?>" size="25" readonly="readonly" />
        </label></td>
        <td>Fecha Asig.</td>
        <td><label>
          <input name="textfield4" type="text" id="textfield4" value="<? echo date("Y-m-d"); ?>" size="25" readonly="readonly" />
        </label>          
	 ...
	 /*?> if($sql="select Id from asignacion_camiones where Id_Camion=".$_POST['id'])
		{
			echo '<script>alert("El Camión '.$_POST[nombre].' ya a sido Asignado a un Trabajador");</script>';
				...
				if($sql="select Id from asignacion_camiones where Id_Trabajador=".$_POST['id2'])
				{
					...
					$sql="insert into asignacion_camiones values('".$p_Id."','".$p_Id_Camion."','".$p_Id_Trabajador."','".$p_Marca_Ca.    				"','".$p_Modelo_Ca."','".$p_Nombre_Ca."','".$p_Patente_Ca."','".$p_Rut_Tra."','".$p_Nombre_Tra."','"  					     				.$p_Fecha_Asignacion."','".$p_Estado."','".$p_Fecha_Modificacion."')";
